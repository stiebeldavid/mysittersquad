...
const BabysitterResponse = () => {
  ...
  const handleVerify = async (mobile: string, email: string) => {
    if (!mobile && !email) {
      toast.error("Please provide either a mobile number or email");
      return;
    }

    try {
      setIsVerifying(true);
      const result = await verifyBabysitterRequest(requestId || "", mobile, email);
